...
import {
  ...
  getCityInfo,
  searchCities,
} from "./fetch"; // Import fetch functions
...
app.get("/search/:location", (req, res) => {
  searchCities(req.params.location).then((data) => {
    ...
    console.log(
      `${cities.length} out of ${data.results.length} cities returned`
    );
...
app.post("/weather", (req, res) => {
  ...
  switch (req.body.type) {
    ...
    default:
      ...
      res
        .status(400)
        .json({ msg: "Case statements not hit. Nothing to see here" });
  }
});
