...
export const getByCity = (location) => {
  return fetch(
    weatherURL() + `?q=${location}&` + "APPID=" + process.env.WEATHER_API_KEY
  ).then((resp) => resp.json());
};
...
export const getByCityId = (id) => {
  return fetch(
    weatherURL() + `?id=${id}&` + "APPID=" + process.env.WEATHER_API_KEY
  ).then((resp) => resp.json());
};

export const searchCities = (city) => {
  return fetch(cityURL() + city + "&key=" + process.env.CITY_API_KEY).then(
    (resp) => resp.json()
  );
};
